class AvatarChatApp {
    constructor() {
        ...
        this.mockResponses = [
            "蔡同学，你上次问的三角函数问题，我记得你当时卡在了第二步。这次我们换个思路，先画个直角三角形看看？",
            "很好！你这个问题问得很棒。正弦定理的公式是 a/sinA = b/sinB = c/sinC，我们一起来看看怎么应用。",
            "你之前说看图解更容易懂，我画了个示意图给你。你看，这个三角形的三个角和三条边是对应的...",
            "这个问题你上周问过类似的，当时卡在了第二步，这次试试换个思路？先从已知条件出发。",
            "你这个月进步很大！上次这类题错误率80%，现在只有20%了。继续保持，我们可以进阶到余弦定理了。",
            "嗨，蔡同学！上次你说的物理作业做完了吗？今天我们继续学三角函数的应用。",
            "别着急，这个知识点确实有点难。我们一步一步来，先理解概念，再做练习。",
            "你之前三角函数总错，现在正确率85%了，可以进阶了！要不要试试综合题？"
        ];
    ...
    setupEventListeners() {
        ...
        document.getElementById('searchKnowledge').addEventListener('keydown', (e) => {
    ...
    getMockResponse(userMessage) {
        ...
        if (lowerMessage.includes('三角函数') || lowerMessage.includes('正弦') || lowerMessage.includes('余弦')) {
            return "三角函数是我们最近的重点内容。你之前说正弦定理有点难理解，我们一起来看看：正弦定理的公式是 a/sinA = b/sinB = c/sinC。这个公式告诉我们，三角形的边长和对应角的正弦值成比例。你哪里不太明白？";
        }
        
        if (lowerMessage.includes('教案') || lowerMessage.includes('备课')) {
            return "好的，我可以帮你生成教案。你可以点击顶部的"教案"按钮，输入教学目标、学生水平和课时长度，我会自动生成结构化的教案，还可以生成配套的PPT和配图哦！";
        }
        
        if (lowerMessage.includes('知识图谱') || lowerMessage.includes('知识点')) {
            return "我们的知识图谱记录了知识点之间的关联关系。比如学正弦定理之前，需要先掌握直角三角形和三角函数定义。点击顶部的"知识图谱"按钮可以看到完整的知识网络！";
        }
        
        if (lowerMessage.includes('记忆') || lowerMessage.includes('记得')) {
            return "当然记得！你叫蔡同学，初二，上次学三角函数时正确率从80%提升到了95%。你的学习风格是视觉型，喜欢看图解。晚上学习效率比较高。这些我都记着呢！";
        }
        
        if (lowerMessage.includes('评价') || lowerMessage.includes('成绩') || lowerMessage.includes('进步')) {
            return "你最近的表现很棒！课堂表现92分，知识点掌握85分，较上月提升了15分。三角函数定义掌握得很好（95%），正弦定理还需要加强（75%）。点击顶部的"评价"按钮可以看到详细分析！";
        }
        
        if (lowerMessage.includes('你好') || lowerMessage.includes('嗨') || lowerMessage.includes('hello')) {
            return "嗨，蔡同学！今天想学点什么？上次我们学的三角函数，你回去复习了吗？有什么不懂的随时问我！";
        }
        
        if (lowerMessage.includes('谢谢') || lowerMessage.includes('感谢')) {
            return "不客气！能帮到你我很开心。记住，学习是一个循序渐进的过程，不要着急，我们一步一步来。有问题随时找我！";
        }
    ...
    startVoiceInput() {
        document.getElementById('voiceInputBtn').style.background = 'linear-gradient(135deg, #ef4444, #f87171)';
    ...
    searchKnowledge() {
        ...
        this.showNotification(`搜索知识点：${searchTerm}`, 'info');
    }
